...
tc = timeit("small_cupy_memory",lambda: convolve2d(cp.array(a),cp.array(k)),timeout=10,gsync=True)
...
print(f"gpu time: {tB:.4e}")
print(f"operation speedup: {tA/tB:.2f}")
tC = timeit("big_cupy_memory",lambda: convolve2d(cp.array(A),cp.array(k)),timeout=30,gsync=True)
print(f"gpu+mem time: {tC:.4e}")
print(f"total speedup: {tA/tC:.2f}")
