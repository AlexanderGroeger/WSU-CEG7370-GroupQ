...
tc = timeit("small_cupy_memory",lambda: gaussian_filter(cp.array(a),1),timeout=10,gsync=True)
...
print(f"gpu time: {tB:.4e}")
print(f"operation speedup: {tA/tB:.2f}")
tC = timeit("big_cupy_memory",lambda: gaussian_filter(cp.array(gA),1),timeout=30,gsync=True)
print(f"gpu+mem time: {tC:.4e}")
print(f"total speedup: {tA/tC:.2f}")
